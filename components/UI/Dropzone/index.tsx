...
export const Dropzone: FC<Props> = (props) => {
	...
	if (files.length > 0 && !isFileListSeparate) {
		return <FileList fileList={files} onDeleteFile={handleDeleteFile} />;
	}
	...
	return (
		<>
			...
			>
				...
				<div className={s.info}>
					...
					{isDragActive ? (
						...
						<p>
							{label} or <span>browse</span> to upload.
						</p>
					)}
			...
			{files.length > 0 && isFileListSeparate && (
				<FileList fileList={files} onDeleteFile={handleDeleteFile} />
			)}
		</>
	);
};
...
const FileItem: FC<{ file: File; onDeleteFile: () => void }> = ({
	file,
	onDeleteFile,
}) => {
	...
	return (
		<div className={s.file_item}>
			...
			<div className={s.file_item_delete} onClick={onDeleteFile}>
				<Image src={trashIcon} alt={'trash_icon'} width={20} height={20} />
			</div>
		</div>
	);
};
