...
import s from '../CreateBusinessAccount/CreateBusinessAccount.module.scss';
...
export const ConvertToBusinessAccount = () => {
	...
	const [isConvertedToBusiness, setIsConvertedToBusiness] = useState<boolean>(false);
	...
	const closeModal = async () => {
		dispatch(setModal(''));
		if (isConvertedToBusiness) logout();
		setStep(1);
	};
	...
	function handleNextStep() {
		if (step === 1) setValueStep2('country', 'Select Country');
		setStep((prev: number) => prev + 1);
	}
	...
	const topBar = () => (
		<div className={s.content_top_bar}>
			...
			{step !== 1 && <h3 className={s.content_top_bar_text}>{''}</h3>}
