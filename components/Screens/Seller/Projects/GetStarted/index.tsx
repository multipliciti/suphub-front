		...
		<div className={s.content_group}>
			<div className={s.main_group}>
				<div className={s.main_logo_bg}>
					...
					/>}
					{status === 'declined' && <div className={classNames(s.main_logo_bg, s.main_logo_bg_alert)}>
						<Image
							alt={'Alert logo'}
							className={s.main_logo }
							width={44}
				...
				<div className={s.main_text_group}>
					...
					{status === 'declined' && <><div className={s.main_title}>We’re sorry</div>
						<div className={s.main_text}>
					Your verification did not pass our review. We will refund your fee within 5-7 business days.
				</div></>
				}
					{status === 'verified' && <><div className={s.main_title}>Congratulation</div>
						<div className={s.main_text}>
			...
			<div className={s.steps}>
				<div className={classNames(s.step, status !== 'notVerified' && s.step_status)}>
					<div className={s.step_logotitle_group}>
						<div className={classNames(s.step_logo, status !== 'notVerified' && s.step_logo_status)}>
							<svg
								className={classNames(s.step_logo_img, status !== 'notVerified' && s.step_logo_img_submitted)}
					...
					{status === 'notVerified' ? <button
						...
						<button
							className={classNames(s.step_btn_status)}
						>
				...
				<div className={classNames(s.step,
					status==='verified' && s.step_status
				)}>
					<div className={s.step_logotitle_group}>
						<div className={classNames(s.step_logo,status==='verified' && s.step_logo_status)}>
							<svg
								xmlns='http://www.w3.org/2000/svg'
								className={classNames(s.step_logo_img, status==='verified' &&s.step_logo_img_submitted)}
								width={40}
								height={40}
							>
								<path
				...
				<div className={s.step}>
					...
					<button
						onClick={(e) => {
							e.preventDefault();
							router.push('/settings/banking-info');
						}}
						className={classNames(s.step_btn_send)}
					>
						<div className={classNames(s.step_btn_text, s.step_btn_text_send)}>
